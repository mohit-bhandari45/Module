...
function reverseCase(str, flag = false) {
    if (typeof str !== "string" || str.length === 0) return "";
...
function reverseInParen(str, flag = false) {
    if (typeof str !== "string" || str.length == 0) return "";

    if (flag) str = str.trim();
    
    return str.replace(/\(([^()]+)\)/g, (_, group) => `(${reverse(group)})`);
}

function removeDups(str, flag = false) {
    if (typeof str !== "string" || str.length === 0) return "";
...
export {
    reverse, reverseEach,
    reverseSen, reverseLetters,
    reverseWordsIntactPuncs, reverseVowels,
    reverseCons, mirrorSen, shuffleWords,
    countChars, reverseCase, removeDups, reverseInParen
};